...
def runStaticGame(h):
    ...
    return runGame(h, [
        ...
        {'url': ENDPOINT+"/settings/${gameId}?player=${player2}", 'status': 200, 'loc': ENDPOINT+"/settings/"},
        {'url': ENDPOINT+"/player/${gameId}?player=${player2}&handle=NEWHANDLE", 'status': 200, 'loc': ENDPOINT+"/game/${gameId}",
         'assertions': [ lambda _, cont: "NEWHANDLE" in cont ]},
